...
router.get(
  ...
  (req, res, next) => {
    ...
    if (rol === 'visitor' && !req.params.id) {
      return res
        .status(400)
        .json({ error: 'Missing parameter: ID is required for visitors.' })
    }
    next()
  },
...
router.put(
  ...
  (req, res, next) => {
    ...
    if (rol === 'visitor') {
      return res.status(403).json({
        error: 'Access denied: Visitors cannot update company information.'
      })
    }
    if (rol === 'co' && req.params.id && req.params.id !== req.user.id) {
      return res.status(403).json({
        error: 'Access denied: Companies can only update their own information.'
      })
    }
    next()
  },
...
router.put(
  ...
  (req, res, next) => {
    ...
    if (rol === 'visitor') {
      return res.status(403).json({
        error: 'Access denied: Visitors cannot delete company documents.'
      })
    }
    if (rol === 'co' && req.params.id && req.params.id !== req.user.id) {
      return res.status(403).json({
        error: 'Access denied: Companies can only delete their own documents.'
      })
    }
    next()
  },
...
router.put(
  ...
  (req, res, next) => {
    ...
    if (rol === 'visitor') {
      return res.status(403).json({
        error: 'Access denied: Visitors cannot update company documents.'
      })
    }
    if (rol === 'co' && req.params.id && req.params.id !== req.user.id) {
      return res.status(403).json({
        error: 'Access denied: Companies can only update their own documents.'
      })
    }
    next()
  },
...
router.post(
  ...
  informationController.addLinksCalendar
)

router.get(
  '/get-links-calendar/:id?',
  verifyToken,
  (req, res, next) => {
    const { rol } = req.user
  ...
  informationController.getLinksCalendar
)
